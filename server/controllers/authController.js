...
export const login = async  (req,res) =>{
    ...
    if(!email || !password){
        return res.json({success:false,message:'Email and password are reurired'})
    }
    try {
        const user = await userModel.findOne({email});
        if(!user){
            return res.json({success:false, messasge:"Invalid Email"})
        }
...
// SENDING verification OTP on the user's email
   ...
   export const verifyEmail = async (req,res)=>{
    ...
    try {
        ...
        if(user.verifyOtpExpireAt < Date.now()){
            return res.jon({success:true, message: "Otp Expired"});
        }
...
export const resetPassword = async(req,res)=>{
    ...
    if(!email || !otp || !newPassword){
        return res.json({suucess:false , message : "All fiels are required"});
    }
    ...
    try {
        ...
        //if otp is valid and not expired then Set The new Password .......
